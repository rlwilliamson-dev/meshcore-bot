...
LOCAL: our alert path resolves the NWS county/forecast zone via a /points lookup
before querying alerts (storm-based polygons miss the exact GPS point even when
the county is warned), so each fresh request makes one more session call than
upstream. The counts below include that lookup.
"""
...
async def test_weather_service_intl_400_skips_after_first_poll(mock_logger):
    ...
    assert call_count == 2  # zone lookup + alerts
...
async def test_weather_service_us_200_sets_available(mock_logger):
    ...
    assert service.api_session.get.call_count == 2  # zone lookup + alerts
...
async def test_weather_service_timeout_retries_next_poll(mock_logger):
    ...
    assert call_count == 3  # zone lookup + alerts, then retry
...
def test_wx_command_us_200_sets_available(mock_logger):
    ...
    assert cmd.noaa_session.get.call_count == 2  # zone lookup + alerts
